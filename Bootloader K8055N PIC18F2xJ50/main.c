...
void Main(void)
{
	...
	// Check the Open8055 BootLoader condition (card address 3) and jump
	// to the user firmware if not met.
	ReadCardAddress();
	if (cardAddress != 3)
	{
		_asm
 ...
 *					sk5 and sk6
 *
 * Overview:		On the K8055N board the card address jumpers sk5 and sk6
...
static void ReadCardAddress(void)
{
	...
	#else
		#error "Processor type not supported (yet) in file __FILE__, line __LINE__"
	#endif
}
